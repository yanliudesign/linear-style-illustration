...
## 为什么不直接用 Mermaid / Excalidraw？

通用画图工具追求的是"快速表达结构"——方框加箭头，主题随便换。这个 skill 追求的是一种非常具体的**视觉风格**：Linear 风格 keynote、AI agent 产品 deck、暗色模式官网上常见的那种深色、发光的"AI 操作系统"质感。

- **Mermaid** 能画出干净的流程图，但不管换什么主题，看起来都还是"流程图"——通用方框、通用箭头，没有发光、没有 orb 背景、没有"内核"式的强调。适合表达逻辑，但不适合放进 keynote 页面。
- **Excalidraw** 快且是手绘风，适合白板式草图，但那种潦草的手绘线条恰恰是这个 skill 精确、技术、冷静风格的反面。
- **给这两个工具切换"暗色模式"** 只是把颜色反相，并不会给你药丸形节点、单一发光焦点，或者一套克制的 2–3 色角色分工系统。

与其说这是个画图工具，不如说它是**为一种具体插画风格定制的设计系统**：固定的颜色角色分工（青色/紫色/淡紫），每张图只有一处发光，节点只用药丸形，交付内联 SVG 的自包含 HTML——所以每次出的图都是"可以直接放进 keynote"的，而不只是"一张图"。
